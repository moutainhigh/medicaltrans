...
public class RedisUtil {
    ...
    abstract class Executor<T> {
        ...
        public Executor(ShardedJedisPool shardedJedisPool) {
            try {
                ...
                //DB选择
                Collection<Jedis> s = jedis.getAllShards();
                Iterator<Jedis> iter = s.iterator();
                while (iter.hasNext()) {
                    Jedis j = (Jedis) iter.next();
                    j.select(3);
                }
     ...
     * @return
     */
    public Integer getSequnces(final String key) {
        return new Executor<Integer>(shardedJedisPool) {
            @Override
            Integer execute() {
                ...
                try {
                    ...
                    if (null == this.jedis.get(key)) {
                        this.jedis.set(key, "10000");
                        currentSeq = 10;
                    } else {
                        currentSeq = Integer.valueOf(jedis.get(key));
                        this.jedis.set(key, String.valueOf(currentSeq + 1));
                    }
                    return currentSeq;
                } finally {
                    lock.unlock();
     ...
     * @return
     */
    public Integer getSequncesForBill(final String key,final String communityId) {
        ...
        return new Executor<Integer>(shardedJedisPool) {
            ...
            Integer execute() {
                ...
                try {
                    Integer currentSeq = null;
                    if (null == this.jedis.hget(key,communityId)) {
                        this.jedis.hset(key,communityId,"2");
                        currentSeq = 1;
                    } else {
                        currentSeq = Integer.valueOf(this.jedis.hget(key,communityId));
                        this.jedis.hset(key,communityId,String.valueOf(currentSeq + 1));
                    }
                    return currentSeq;
                } finally {
                    lock.unlock();
     ...
     * @return
     */
    public Long getSequncesLong(final String key) {
            ...
            Long execute() {
                ...
                try {
                    ...
                    if (null == this.jedis.get(key)) {
                        ...
                        currentSeq = Long.valueOf(jedis.get(key));
                        this.jedis.set(key, String.valueOf(currentSeq + 1l));
                    }
                    return currentSeq;
                } finally {
                    lock.unlock();
     ...
     * @return
     */
    public Integer getSequncesByCall(final String key) {
        return new Executor<Integer>(shardedJedisPool) {
            @Override
            Integer execute() {
                RedisLock lock = new RedisLock(key, shardedJedisPool);
     ...
     * @return 返回生成的Id
     */
    public String incrSetZero(final String key) {
        return new Executor<String>(shardedJedisPool) {
            @Override
            String execute() {
                return jedis.getSet(key, "0");
            }
        }.getResult();
    }
    ...
    /**
     * 返回有序集 key 中， score 值介于 min 和 max 之间(默认包括等于  min 或max )的所有的成员。
     * 有序集成员按 score 值递减(从小到大)的次序排列。
     *
     * @param key key
     * @param min score最小值
     * @param max score最大值
     * @return 指定区间内，带有 score 值(可选)的有序集成员的列表
     */
    public Set<String> rangeByScoreWithSortedSet(final String key, final double min, final double max) {
        return new Executor<Set<String>>(shardedJedisPool) {
    ...
    /**
     * 删除有序集 key 中， score 值介于 max 和 min 之间(默认包括等于 max 或 min )的所有的成员。
     *
     * @param key key
     * @param max score最大值
     * @param min score最小值
     ...
     */
    public Long remrangeByScoreWithSortedSet(final String key, final double max, final double min) {
        return new Executor<Long>(shardedJedisPool) {

            @Override
            Long execute() {
                return jedis.zremrangeByScore(key, max, min);
            }
        }.getResult();
    }

    /**
     * 删除有序集 指定的key的指定member
     *
     * @param key key
     * @return 被成功移除的成员的数量，不包括被忽略的成员
     */
    ...
    public long incrBy(final String key, final long integer) {
        return new Executor<Long>(shardedJedisPool) {
            ...
            @Override
            Long execute() {
                long id = jedis.incrBy(key, integer);
                return id;
            }
        }.getResult();
    }
    ...
    public long decrBy(final String key, final long integer) {
        return new Executor<Long>(shardedJedisPool) {
            ...
            @Override
            Long execute() {
                long id = jedis.decrBy(key, integer);
                return id;
            }
        }.getResult();
    }
    ...
    /**
     * 向key指定的set中添加元素value,如果key不存在则创建
     *
     * @param
     * @return long
     * @auth FuShengcai
